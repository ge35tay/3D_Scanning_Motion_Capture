...
bool WriteMesh(Vertex* vertices, unsigned int width, unsigned int height, const std::string& filename)
{
	...
	unsigned nFaces = 0;
	unsigned maxNFaces = (width-1)*2 * (height-1);  // max 2 * width * height
...
int main()
{
	...
	std::string filenameIn = "/home/yinghanhuang/TUM/3DMotion/Data/rgbd_dataset_freiburg1_xyz/";
		...
		for (int row = 0; row < sensor.GetDepthImageHeight(); row++)
		{
			for (int column=0; column<sensor.GetDepthImageWidth(); column++)
			{
				...
				if (depthMap[idx] == MINF){
					...
					float current_depth = depthMap[idx];
					Vector3f p_image_frame = Vector3f(current_depth * column, current_depth * row, current_depth);
					Vector4f p_camera_frame = extendedId * depthIntrinsicsInv * p_image_frame;
					Vector4f p_world_frame = trajectoryInv * depthExtrinsicsInv * p_camera_frame;
					p_world_frame[3] = 1.0f;
					vertices[idx].position = p_world_frame;
